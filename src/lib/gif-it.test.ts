...
describe("video format helpers", () => {
  it("accepts supported video extensions", () => {
    ...
    expect(isSupportedVideo(createFile("clip.webm"))).toBe(true);
  });
  ...
  it("accepts supported video MIME types", () => {
    ...
    expect(isSupportedVideo(createFile("clip", "video/webm"))).toBe(true);
  });
  ...
  it("rejects unsupported extensions and MIME types", () => {
    ...
    expect(isSupportedVideo(createFile("clip", "video/avi"))).toBe(false);
  });
});
